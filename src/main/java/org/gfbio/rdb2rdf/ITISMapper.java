...
public class ITISMapper implements Mapper {
  ...
  public void exportTablesToJSON() {
    ...
    for (MappingResource resource : mappingConfig.getResources()) {
       ...
       *
       * double batches = Math.ceil(rows / Integer.valueOf(props.getQueryLimit())) +
       * 1;
       *
       ...
       * StringBuilder().append(props.getWorkDir()).append(ONTO).append(File.
       * separator)
       * .append(resource.getTable()).append("-part-" + i + "-").append(".json");
       *
       * set = mysqlIf.select(resource.getTable(), ONTO, String.valueOf(offset));
       *
       * mysqlIf.exportToJSON(set, resource.getTable(), filename.toString(), ONTO,
       * false);
       *
       * set.close();
       *
       ...
       *
       * StringBuilder filename = new
       * StringBuilder().append(props.getWorkDir()).append(ONTO)
  ...
  private void addTypeOwlClassStatement(String tableName, Model model) {
    ...
    if (!tableToClass.get(tableName).equalsIgnoreCase("")) {
      ...
      Statement statement = model.createStatement(model.createResource(baseUri + "/" + tableToClass.get(tableName)),
   ...
   * <p>
   * http://terminologies.gfbio.org/ITIS/Taxa_16648
   * http://www.w3.org/1999/02/22-rdf-syntax-ns#type
